...
if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
  if (!config.SESSION_ID) return console.log('❗ [NETHUM-MD] SESSION_ID not found in env. Please configure it.');
  const sessdata = config.SESSION_ID;
  const filer = File.fromURL(`https://mega.nz/file/${sessdata}`);
  filer.download((err, data) => {
    if (err) throw err;
    fs.writeFile(__dirname + '/auth_info_baileys/creds.json', data, () => {
      console.log("📥 [NETHUM-MD] Session file downloaded and saved.");
    });
  });
}
...
async function connectToWA() {
  console.log("🛰️ [NETHUM-MD] Initializing WhatsApp connection...");
  ...
  conn.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close' && lastDisconnect?.error?.output?.statusCode !== DisconnectReason.loggedOut) {
      ...
      console.log("🔧 [NETHUM-MD] Installing plugins...");
      ...
      console.log("✅ [NETHUM-MD] Plugins installed successfully.");
      console.log("📶 [NETHUM-MD] Successfully connected to WhatsApp!");
...
app.get("/", (req, res) => {
  res.send("Hey, NETHUM-MD started✅");
});

app.listen(port, () => console.log(`🌐 [NETHUM-MD] Web server running → http://localhost:${port}`));
